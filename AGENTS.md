...
- The owner explicitly changed distribution to paid upfront on 2026-09-22. App Store handles payment before download; every installed app has all features, with no in-app purchase, subscription, paywall, restore-purchase action or premium switch. Keep local data independent of store payment. No own card form, wallet or secret billing service.
...
- Use bundle and App Group IDs as configurable candidates only; owner must authorize actual identifiers. Do not recreate the superseded non-consumable product. Never put tokens, certificates or personal data in the repository.
- Do not publish, create paid resources, submit agreements or deploy to a real account without explicit authorization.

## Definition of done

A task is done only if its acceptance checks have evidence. Syntax parsing is not typechecking; TestFlight is not evidence of an actual paid App Store sale; browser render is not native UI QA. Update task status and `STATUS.md` with factual results. Preserve failed tests and unresolved blockers instead of deleting them.
